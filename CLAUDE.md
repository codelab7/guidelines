...
- `AI/code-guideline/` — the AI agent specs plus starter folder structures. `GENERAL_GUIDELINE.md` is the global agent-behaviour spec (communication style, planning, ambiguity handling, error correction). `PHP_LARAVEL_GUIDELINE.md` and `REACT_GUIDELINE.md` are framework-specific specs that explicitly build on it. Alongside them, `laravel/`, `react/`, and `laravel-react/` are folder trees showing where code belongs in each stack; every folder in them holds a `CLAUDE.md` carrying that folder's rules. `laravel/` is filled in and is the source of truth for the Laravel rules, which is why `PHP_LARAVEL_GUIDELINE.md` is only an index pointing at those files; `react/` and `laravel-react/` are still blank placeholders. Keep a scaffold's folders and the spec that documents them in sync — if one gains a folder, the other needs updating.
