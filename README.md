...
The same folder holds starter folder structures to copy into a new project, one per stack: [`laravel/`](AI/code-guideline/laravel/), [`react/`](AI/code-guideline/react/), [`laravel-react/`](AI/code-guideline/laravel-react/), and [`react-native/`](AI/code-guideline/react-native/). Each folder in them carries a `CLAUDE.md` with the rules for that folder, so an agent picks up the right rules from where it is working. All four trees are filled in, and each one's `README.md` ends with a checklist of rules still to settle. See [`AI/code-guideline/README.md`](AI/code-guideline/README.md) for how they fit together.
